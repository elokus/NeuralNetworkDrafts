...
class DataPreprocessor():
    ...
    def save_all_files(self, prefix="BTCUSDT", save_only_originals=False):
        ...
        if self.original_dfs is not None:
...
#############################################################################
#######
#######------MODEL PIPELINE v2 loading from files after FEATURE PIPELINE output tuple--------
#######
#######
##############################################################################
...
class FileExtractor():
    ...
    def __init__(self, input_folder, prefix, timeframes, start_date, end_date=None, transformed=True):
        self.path = input_folder
        # transformed data: "_BTCUSD", original data: "BTCUSD" if set true only transformed are loaded else only original
        if transformed:
    ...
    def load_files_from_filename(self):
        for t in self.timeframes:
            filename = f"{self.path}/{self.prefix}_{t}.csv"
            try:
                df = pd.read_csv(filename, parse_dates=True)
            except:
                print(f"File not found check {filename}")
                return
            df["Date"] = pd.to_datetime(df["Date"])
            df = df.set_index("Date").sort_index()[self.start_date:self.end_date]
            df.columns = df.columns.str.lower()
            self.data.append((t,df))
            self.data_dict[t] = df

        def plot_data(self, column="Close"):
            fig, axs = plt.subplots(len(self.dataframes), 1, figsize=(15, 6))
            i = 0
            for name, df in self.data:
                axs[i].plot(df[column])
                axs[i].set_title(f'Timeseries {name}')
                plt.subplots_adjust(bottom=0.01)
            plt.show()
    def __repr__(self):
        strings = ''
        for (name, df) in self.data:
            string = name + ":"
            strings += string + str(df.shape) + " | "
        return '\n'.join([
            f'Total dataframes: {len(self.data)}',
            'Dataframe Name/Shapes:',
            f'{strings}'])
