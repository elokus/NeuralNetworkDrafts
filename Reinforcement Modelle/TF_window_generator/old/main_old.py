...
CONV_INPUT_WIDTH = 12
CNN_INPUT_WIDTH = 12 + CONV_INPUT_WIDTH-1 #to plot cnn INPUT_WIDTH = LABEL_WIDTH + (CONV_WIDTH -1)
...
cnn_window = WindowGenerator(input_width=CNN_INPUT_WIDTH, label_width=12, shift=1, train_df=train_df, val_df=val_df, test_df=test_df, label_columns=["label"])
...
val_performance = {}
test_performance = {}
for name, model, wd in models:
    print(f">>>>>>>>   {name}")
    history = compile_and_fit(model, wd, MAX_EPOCHS=100)
