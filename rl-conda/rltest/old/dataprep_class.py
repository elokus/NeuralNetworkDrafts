    ...
    def parse_links(self):
        ...
        for link in links:
            if link.endswith('.csv'):
                ...
                if tframe == "d":
                    daily[symbol] = dlink
                    hourly[symbol] = dlink.replace("_d", "_1h")
                    minute[symbol] = dlink.replace("_d", "_minute")
        self.d = daily
        self.h = hourly
        self.m = minute
    ...
    def clean(self):
        ...
        if self.d is not None:
            ...
            df = pd.DataFrame.from_dict({"Date": self.d["Date"], "Open": self.d["open"],
                                         "High": self.d["high"], "Low": self.d["low"], "Close": self.d["close"], "Volume": self.d["Volume USDT"], "Amount": self.d["tradecount"]})
        ...
        if self.h is not None:
            ...
            df = pd.DataFrame.from_dict({"Date": self.h["Date"], "Open": self.h["open"],
                                         "High": self.h["high"], "Low": self.h["low"], "Close": self.h["close"], "Volume": self.h["Volume USDT"], "Amount": self.h["tradecount"]})
            df.set_index("Date", inplace=True)
            self.h = df.sort_index()

        if self.m is not None:
            self.m["Date"] = pd.to_datetime(self.m["unix"]/1000, unit="s")
            df = pd.DataFrame.from_dict({"Date": self.m["Date"], "Open": self.m["open"],
                                         "High": self.m["high"], "Low": self.m["low"], "Close": self.m["close"], "Volume": self.m["Volume USDT"], "Amount": self.m["tradecount"]})
            df.set_index("Date", inplace=True)
...
#    print(linklist.m)
    BTCUSDT = Import_Data(linklist, "BTCUSDT", daily=True,
                          hourly=False, minutes=False)

    BTCUSDT.clean()
    # print(df)
    # df = add_mooncycle(df)
    #return BTCUSDT.d, BTCUSDT.h, BTCUSDT.m
    return BTCUSDT.d
