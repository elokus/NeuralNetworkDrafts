...
class cust():
    ...
    def outlier_removal(self, var):
        ...
        self.dat = self.dat[(self.dat[var] > min_val)
                            & (self.dat[var] < max_val)]
    ...
    def comparison_df(y_pred, y_test):
        ...
        comparison_df['abs_difference'] = abs(
            comparison_df['y_pred'] - comparison_df['y_test'])
        comparison_df['real_difference'] = comparison_df['y_pred'] - \
            comparison_df['y_test']
        print(comparison_df.describe())
        # Show all sums
        print(comparison_df.sum())
        # Show average difference
        print("Average Difference: ", comparison_df.sum()
              [2] / len(comparison_df))
